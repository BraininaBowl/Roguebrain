function actor(x, y, type) {
	...
	this.move = function (x, y) {
		...
		document.querySelectorAll(".mapcell." + type).forEach((item) => item.remove());
		this.x = x;
		this.y = y;
	...
	this.create = function () {
		...
		this.addStream = function () {
			...
			while (stream) {
				...
				if (dir == 0 && y < height) {
					y++;
					for (let ix = -streamwidth_offset_1; ix < streamwidth_offset_2; ix++) {
						this.setCell(x + ix, y, "water");
					}
				} else if (dir == 1 && x < width) {
					x++;
					for (let iy = -streamwidth_offset_1; iy < streamwidth_offset_2; iy++) {
		...
		this.addPoi = function () {
			...
			let poi_template = pois_lut[pois_lut_keys[getRandomInt(pois_lut_keys.length)]];
		...
		for (let i = 0; i < iteration; i++) {
			...
			for (let i2 = 0; i2 < 3; i2++) {
				this.addPath(getRandomInt(width), getRandomInt(height), getRandomInt(40) + 40, "clear");
			}
		}
